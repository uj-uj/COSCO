...
class DMARLScheduler(Scheduler):
    def __init__(self, data_type, training=True):
        super().__init__()
        self.models = [eval(data_type + "_RL")() for _ in range(3)]  # Create a list of models for each agent
        self.optimizers = [torch.optim.AdamW(model.parameters(), lr=0.0001, weight_decay=1e-5) for model in self.models]
        self.training = training
        self.hosts = int(data_type.split('_')[-1])
        self.last_schedule = None
        self.models, self.optimizers, self.epoch, self.accuracy_list = load_model(data_type + '_RL', self.models[0], data_type)
        self.data_type = data_type
        self.buffer = []
        directory = 'scheduler/A3C/dataset/'
    ...
    def run_DMARL(self, containerIDs, rnd):
        self.epoch += 1
        schedule_next, prev_alloc = self.get_current_schedule()
        value_t = self.get_last_value() if self.last_schedule is not None else 0.5
    ...
    def selection(self):
        # Implement EDF (Earliest Deadline First) selection policy
        selected_containers = []
